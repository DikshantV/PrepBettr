...
# Array of all public assets (just the filename part for easier matching)
assets=(
    "ProductMockup1.png"
    "ProductMockup2.png"
    "YTThumbnail.png"
...
for asset in "${assets[@]}"; do
    ...
    # Look for both bare filename and path-based references
    found=false
    
    # Check for exact filename match
    if rg -F "$asset" -g "!public/**" . >/dev/null 2>&1; then
        found=true
    fi
    
    # Check for path-based reference (e.g., /asset or /folder/asset)
    if rg -F "/$asset" -g "!public/**" . >/dev/null 2>&1; then
        found=true
    fi
    
    # For assets in subdirectories, check for the full path
    if [[ "$asset" == *"/"* ]]; then
        if rg -F "/$asset" -g "!public/**" . >/dev/null 2>&1; then
            found=true
        fi
