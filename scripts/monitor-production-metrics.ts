...
import { MonitorQueryClient, MetricsQueryResult } from '@azure/monitor-query';
...
class ProductionMetricsMonitor {
  private queryClient: MonitorQueryClient;
  private config: AlertConfig;
  private thresholds: MetricThresholds;

  constructor() {
    this.queryClient = new MonitorQueryClient(new DefaultAzureCredential());
    this.config = {
  ...
  async checkApplicationInsightsErrorRate(): Promise<{ errorRate: number; isAlertCondition: boolean }> {
    ...
    try {
      const result = await this.queryClient.queryWorkspace(
  ...
  async checkFrontDoorMetrics(): Promise<{ errorRate: number; avgLatency: number; isErrorAlert: boolean; isLatencyAlert: boolean }> {
    ...
    try {
      ...
      const errorMetrics = await this.queryClient.queryResource(
        resourceUri,
      ...
      // Query for latency
      const latencyMetrics = await this.queryClient.queryResource(
        resourceUri,
        ['TotalLatency'],
        {
          granularity: 'PT1M', 
