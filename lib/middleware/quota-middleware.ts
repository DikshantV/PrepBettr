...
/**
 * Enhanced withQuota wrapper with caching for better performance
 */
export function withQuotaAndCache(options: WithQuotaOptions) {
  // Simple in-memory cache for quota checks (5 minute TTL)
  const cache = new Map<string, { result: QuotaCheckResult; expiry: number }>();
  ...
  return function(handler: (req: NextRequest, context?: any) => Promise<NextResponse>) {
    return async function(req: NextRequest, context?: any): Promise<NextResponse> {
      try {
        ...
        const userId = verificationResult.decodedToken.uid;
        const customUsageDocId = options.usageDocId || userId;
        const cacheKey = `${customUsageDocId}:${options.featureKey}`;
        ...
        if (cached && cached.expiry > Date.now()) {
          quotaResult = cached.result;
        } else {
          // Cache miss or expired - check quota
          quotaResult = await checkUserQuota(
            customUsageDocId,
            options.featureKey,
            options.limitFree
          );
        ...
        // If successful, increment counter and invalidate cache
        if (response.status >= 200 && response.status < 300) {
          // Invalidate cache immediately after successful usage
