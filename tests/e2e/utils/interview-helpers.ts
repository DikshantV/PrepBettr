...
export const RESPONSE_SETS = {
  technical: {
    ...
    mid: [
      "I would analyze the problem complexity and choose appropriate data structures for efficiency.",
      "This looks like it could be solved with a two-pointer technique or hash map for O(n) performance.",
      "I would implement the solution with proper error handling and consider optimization opportunities.",
    ],
    senior: [
  ...
  behavioral: {
    ...
    mid: [
      "I recently led a small team project where we delivered a feature ahead of schedule.",
      "When I encounter conflicts, I facilitate discussions to find win-win solutions for all parties.",
      "I'm motivated by solving complex problems and mentoring newer team members.",
    ],
    senior: [
  ...
  systemDesign: {
    ...
    mid: [
      "I would design a scalable REST API with proper authentication and data validation.",
      "For the database, I would consider both relational and NoSQL options based on data patterns.",
      "I would implement caching and basic monitoring to ensure good performance from the start.",
    ],
    senior: [
...
export function getResponsesForScenario(
  ...
  const responses = RESPONSE_SETS[responseKey];
  
  if (responses) {
    // Now TypeScript knows that responses has 'entry', 'mid', and 'senior' properties
